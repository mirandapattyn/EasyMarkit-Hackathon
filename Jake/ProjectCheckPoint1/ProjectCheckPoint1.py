...
class DataScienceModeler:
    ...
    def experiment(self):
        ...
        model_classification = sklearn.tree.DecisionTreeClassifier()
        model_classification = model_classification.fit(self.c_x_train, self.c_y_train)

        #### #### #### #### Regression Model #### #### #### ####
        model_regression = sklearn.tree.DecisionTreeRegressor()
    ...
    def experiment_tau(self):
        ...
        #### #### #### #### Prediction Process #### #### #### ####
        y_pred_prob = model_classification.predict_proba(self.c_x_test)[:, 1]

        TAU_LIST = np.arange(0.01, 0.99, 0.01)

        for TAU in TAU_LIST:
            y_pred = np.where(y_pred_prob > TAU, 1, 0)

            confusion_matrix = sklearn.metrics.confusion_matrix(self.c_y_test, y_pred)
            print("Confusion Matrix:")
            print(confusion_matrix)

            f1_score = sklearn.metrics.f1_score(self.c_y_test, y_pred)
            print("F1 Performance Score: %.6f%%" % (f1_score * 100))

            pred_list.append(f1_score)

        print(pred_list)


        plt.plot(TAU_LIST, pred_list, label='F1 Score')
        plt.title('Tau Processing with Decision Tree')
        plt.xlabel('Tau')
        plt.ylabel('F1 Score')
        plt.axvline(0.5, color="red", label="Default F1: 28.54%")
        plt.legend()
        plt.show()


    def experiment_vis(self):
        ...
        confusion_matrix = sklearn.metrics.confusion_matrix(self.c_Y, self.c_Y)
        print("Confusion Matrix:")
        print(confusion_matrix)


        plt.hist(self.c_Y)
        plt.title('Frequencies of 0 (Non-Claimed) and 1 (Claimed)')
        plt.xlabel('0 and 1')
    ...
    def assess(self):
        ...
        # Predict the Claimed and ClaimAmount
        y_pred_classification = model_classification.predict(self.TESTSET_X)
...
if __name__ == "__main__":
    ...
    DataScienceModeler().experiment_vis()
