...
class DataScienceModeler:
    ...
    def experiment01(self, PARAMETER_TO_EXPLORE):
        ...
        y_pred_regression = model_regression.predict(self.r_x_test)
        y_pred_regression = pd.DataFrame(y_pred_regression)[0].values
        ...
        for CA in RANGE:
            y_pred_regression = np.where(y_pred_regression > 0, CA, 0)

            y_pred_final = y_pred_classification * y_pred_regression
            MAE = self.print_regression_performance_metrics(self.r_y_test, y_pred_final)

            RESULT.append(MAE)


        plt.plot(RANGE, RESULT, label='MAE')
        plt.title('Regression with a Constant Value')
